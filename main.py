...
def extract_statistics(post_list):
	...
	for i in range(0, num_of_posts):
		...
		if post_list[i]['caption'] == None: # check if there is caption
			...
			caption_list.append([insta_id,post_list[i]['code'], post_list[i]['caption']['text'].replace("\n", " "), int(time.time())]) 
...
def write_account_bio(insta_id):
	'''
	INTAKE: an Instagram id
	RETURN: a csv file with account bio
	'''
	...
	bio_file = open(insta_id + '_account_bio.csv','w')
	bio_writer = csv.writer(bio_file, encoding = 'utf-8')
	bio_writer.writerow(['insta_id','bio','ts'])
	bio_writer.writerow([insta_id,bio,int(time.time())])

def write_account_stats(insta_id):
	'''
	INTAKE: an Instagram id
	RETURN: a csv file with account bio
	'''
	...
	bio_file = open(insta_id + '_account_stats.csv','w')
	bio_writer = csv.writer(bio_file, encoding = 'utf-8')
